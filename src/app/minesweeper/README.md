...
- **設備自動偵測**: 基於 User Agent 和螢幕寬度智能判斷設備類型
- **動態配置載入**: 根據設備類型自動選擇對應的難度配置
- **UI 適應性**: 按鈕大小、格子尺寸、字體大小自動調整
- **操作模式切換**: PC版傳統右鍵操作，手機版模式按鈕操作
...
- **設備偵測服務**: 自動識別手機/PC設備類型
- **動態配置系統**: `getDifficultyConfigs(isMobile)` 函數動態載入對應配置
- **響應式UI組件**: 根據設備類型調整按鈕大小和佈局
- **操作模式切換**: 統一的點擊處理邏輯，根據設備和模式執行不同動作
